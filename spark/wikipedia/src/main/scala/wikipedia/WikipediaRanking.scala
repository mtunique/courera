...
object WikipediaRanking {
  ...
  val wikiRdd: RDD[WikipediaArticle] = sc.textFile(getClass.getResource("/wikipedia/wikipedia.dat").getPath)
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = {
    rdd.flatMap( wikiArticle => {
      val text = wikiArticle.text
      langs.flatMap( lang => {
        if ( text.startsWith(lang + " ") ||
          text.endsWith(" " + lang) ||
          text.contains(" " + lang + " "))
          List((lang, wikiArticle)) else None
      })
    }).groupByKey()
  ...
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] = {
    index.map( item => (item._1, item._2.size) ).collect()
